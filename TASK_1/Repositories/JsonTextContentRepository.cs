...
namespace TASK_1.Repositories
{
    public class JsonTextContentRepository : ITextContentRepository
    {
        ...
        private List<TextContent> LoadContent()
        {
            ...
            {
                throw new InvalidOperationException($"Error parsing content data: {ex.Message}", ex);
            }
            catch (IOException ex)
            {
                throw new InvalidOperationException($"Error reading content data file: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
        ...
        public void SaveChanges()
        {
            ...
            {
                throw new InvalidOperationException($"Error serializing content data: {ex.Message}", ex);
            }
            catch (IOException ex)
            {
                throw new InvalidOperationException($"Error writing to content data file: {ex.Message}", ex);
            }
            catch (Exception ex)
            {
        ...
        public void ResetAllData()
        {
            try
            {
                ...
                // Clean up any temporary files in the same directory
                string? directory = Path.GetDirectoryName(_filePath);
            ...
            {
                throw new InvalidOperationException($"Failed to reset data: {ex.Message}", ex);
            }
        }
    }
} 